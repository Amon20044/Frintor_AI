...
export async function GET(req: NextRequest) {
    console.log("Received request for horoscope");
  ...
  try {
    ...
    const testData = await getHoroscope(uuid);

    if (!testData) {
      return NextResponse.json({ message: 'Horoscope not found or failed to fetch' }, { status: 404 });
    }
    console.log('Fetched horoscope data:', testData); // Log the fetched data for debugging
